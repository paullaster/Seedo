...
import { isFarmer, Farmer, ProduceCollection, MarketRate } from '@/app/lib/types';
...
export default function FarmerDashboardView({
  ...
  weather: { temp: number; condition: string; humidity: number; forecast: string };
  agents: any[];
  ...
  useEffect(() => {
    if (user && isFarmer(user) && user.location) {
      ...
      agents.forEach((agent: any) => {
        const agentLoc = agent.location || { lat: -1.2921, lng: 36.8219 };
  ...
  return (
    <Box>
          ...
          >
            <CardContent sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              <Box>
                ...
                <Typography variant="h3" fontWeight="900">{weather.temp}°C</Typography>
                <Typography variant="body2" fontWeight="500">{weather.forecast}</Typography>
              </Box>
      ...
      <Grid container spacing={3}>
        <Grid size={{ xs: 12, md: 8 }}>
          <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2 }}>
            ...
            <Button variant="outlined" size="small" onClick={() => router.push('/farmer/payments')}>View Full History</Button>
          </Box>
          <Stack spacing={2}>
              ...
              >
                <CardContent sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', p: '16px !important' }}>
                  <Stack direction="row" spacing={2} alignItems="center">
                    ...
                    <Box>
                      <Typography variant="subtitle1" fontWeight="bold">Invoice #{collection.id.split('-').pop()}</Typography>
                      <Typography variant="body2" color="text.secondary">
                         {collection.produceType} &bull; {collection.weightKg}kg &bull; Grade {collection.grade}
                      </Typography>
                  ...
                  <Box sx={{ textAlign: 'right' }}>
                    ...
                    <Stack direction="row" spacing={1} justifyContent="flex-end" alignItems="center" mt={0.5}>
                      ...
                      />
                      <Button size="small" sx={{ minWidth: 0, p: 0.5 }} onClick={() => handleViewInvoice(collection)}>
                        View
                      </Button>
                    </Stack>
                  </Box>
          ...
          <MotionCard sx={{ height: '100%', bgcolor: 'background.paper', border: `1px solid ${theme.palette.primary.main}30` }}>
            <CardContent>
              ...
              <Stack spacing={2} mt={2}>
                <Button variant="contained" fullWidth startIcon={<AddIcon />} size="large" sx={{ py: 2 }} onClick={() => router.push('/farmer/harvest')}>
                  New Harvest Notice
                </Button>
                <Button variant="outlined" fullWidth size="large">My QR Code</Button>
              </Stack>
              <Box sx={{ mt: 4, p: 2, borderRadius: 2, bgcolor: 'rgba(255,255,255,0.03)' }}>
                <Typography variant="caption" color="text.secondary" display="block" mb={1}>FARMER PROFILE</Typography>
                <Typography variant="body2" color="primary.main">✓ Verified Account</Typography>
                <Typography variant="body2" color="text.secondary" mt={0.5}>
                  {isFarmer(user) ? (user.location?.address || 'Location not set') : ''}
                </Typography>
                <Typography variant="body2" color="text.secondary">ID: {isFarmer(user) ? user.nationalId : ''}</Typography>
              </Box>
            </CardContent>
          </MotionCard>
        </Grid>
      ...
      <Dialog open={openDialog} onClose={() => setOpenDialog(false)} PaperProps={{ sx: { borderRadius: 4, width: '100%', maxWidth: 500 } }}>
        {selectedInvoice && (
          <>
            ...
            <DialogActions sx={{ p: 3 }}>
              ...
              <Button variant="contained" startIcon={<EmailIcon />} onClick={handleResendReceipt}>Resend Receipt</Button>
            </DialogActions>
          </>
        )}
      </Dialog>
